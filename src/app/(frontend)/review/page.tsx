...
export const metadata: Metadata = {
  ...
  description:
    "Share your travel feedback and experience with the MiAfrica concierge team.",
  robots: {
    index: false,
    follow: false,
  },
      ...
      <Card className="border border-border bg-card p-6 sm:p-10 rounded-2xl shadow-sm">
        <CardContent className="p-0 space-y-6">
          ...
          <Suspense fallback={<ReviewFormSkeleton />}>
            <ReviewFormSection searchParams={searchParams} />
          </Suspense>
...
async function ReviewFormSection({
  ...
  const [params, tours] = await Promise.all([
    searchParams,
    getPublishedTours(),
  ]);
  ...
  return <ReviewForm tours={tourOptions} initialTourId={initialTourId} />;
}
